...
async function wait(timeout) {
    if (!timeout || timeout <= 0) {
        if (agentStatus?.paused) await pause();
        return;
    }
...
function showSkipDisabledModal(f) {
    ...
    modalButton.addEventListener('click', () => {
        modalOverlay.remove();
        window.open("https://app.liftmycv.com/upgrade");
    });
...
let ensureAgentPanelAbovePageModalsTimer = null;

function supportsAgentPanelPopover() {
    return typeof HTMLElement.prototype.showPopover === 'function';
}

function isAgentPanelPopoverOpen(blockEl) {
    try {
        return blockEl?.matches?.(':popover-open') === true;
    } catch {
        return false;
    }
}

function hideAgentPanelPopover(blockEl) {
    try {
        if (blockEl?.hidePopover && isAgentPanelPopoverOpen(blockEl)) {
            blockEl.hidePopover();
        }
    } catch (e) {
        console.error('hideAgentPanelPopover failed', e);
    }
}

function presentAgentPanel(blockEl) {
    try {
        if (!blockEl || agentStatus.closed) {
            return;
        }
        if (blockEl.style.display === 'none') {
            return;
        }
        if (supportsAgentPanelPopover()) {
            if (!isAgentPanelPopoverOpen(blockEl)) {
                blockEl.showPopover();
            }
        }
    } catch (e) {
        console.error('presentAgentPanel failed', e);
    }
}

function ensureAgentPanelAbovePageModalsNow() {
    try {
        const blockEl = document.getElementById(STATUS_BLOCK_ELEMENT_ID);
        if (!blockEl || blockEl.style.display === 'none' || agentStatus.closed) {
            return;
        }
        if (!supportsAgentPanelPopover()) {
            return;
        ...
        const foreignDialogs = [...document.querySelectorAll('dialog[open]')];
        if (!foreignDialogs.length && isAgentPanelPopoverOpen(blockEl)) {
            return;
        }

        hideAgentPanelPopover(blockEl);
        blockEl.showPopover();
    } catch (e) {
        console.error('ensureAgentPanelAbovePageModalsNow failed', e);
    }
}

function ensureAgentPanelAbovePageModals() {
    try {
        clearTimeout(ensureAgentPanelAbovePageModalsTimer);
        ensureAgentPanelAbovePageModalsTimer = setTimeout(ensureAgentPanelAbovePageModalsNow, 50);
    } catch (e) {
        console.error('ensureAgentPanelAbovePageModals failed', e);
    }
}

function startAgentPanelModalWatcher() {
    try {
        if (agentStatus.panelModalWatcherStarted) {
            return;
        }
        agentStatus.panelModalWatcherStarted = true;
        ...
        const observer = new MutationObserver(() => {
            try {
                ...
                const foreignDialogs = [...document.querySelectorAll('dialog[open]')];
                if (foreignDialogs.length) {
                    ensureAgentPanelAbovePageModals();
                }
            } catch (e) {
                console.error('agent panel modal watcher failed', e);
            }
        });
        ...
        console.error('startAgentPanelModalWatcher failed', e);
    }
}
...
function warmingUp(geometry, agentMessages, agentMode) {
    if (agentStatus.alreadyWarmed) {
        ...
        if (blockElOld) {
            blockElOld.style.removeProperty('display');
            presentAgentPanel(blockElOld);
            ensureAgentPanelAbovePageModals();
        }
        return;
    }
    ...
    closeBtn.style.zIndex = '2147483646';
    ...
    minimizeBtn.style.zIndex = '2147483646';
    ...
    styleEl.innerHTML = `
        ...
        #${STATUS_BLOCK_ELEMENT_ID}:popover-open {
            display: flex;
        ...
        #${STATUS_BLOCK_ELEMENT_ID}:focus,
        #${STATUS_BLOCK_ELEMENT_ID}:focus-visible {
            border: none;
            outline: none;
        }
    ...
    function saveAgentGeometry() {
        const styledHeight = parseInt(blockEl.style.height, 10) || 0;
        const { top, left, width, height } = blockEl.getBoundingClientRect();
        const g = { top, left, width, height };
        if (g.height <= 0 && styledHeight > 0) {
            g.height = styledHeight;
        }
        if (g.top < 0) {
            blockEl.style.top = 0;
        }
        ...
        if (g.height > 0 && g.height < 120 && !agentStatus.search) {
    ...
    switchCopilotBtn.addEventListener('click', function(e) {
        ...
        chrome.runtime.sendMessage({type: 'SET-HAS-MANUAL-EDITS'});
    ...
    if (!agentMessages || !agentMessages.length) {
        if (agentMessages !== false) {
            if (agentMode != 'Copilot') {
                appendStatusMessage('WARNING! Do not perform any manual actions or minimize this browser window while the AI agent is operating. You’ll be able to review and adjust fields only after switching to Copilot mode at the end of the autofill process.');
            } else {
                appendStatusMessage('WARNING! Do not perform any manual actions or minimize this browser window while the AI agent is operating. Since you’re in Copilot mode, you’ll be able to review and adjust fields at the end of the autofill process.');
            }
        }
    } else {
    ...
    presentAgentPanel(blockEl);
    startAgentPanelModalWatcher();
    saveAgentGeometry();
...
async function challengeFound() {
    ...
    chrome.runtime.sendMessage({type: "CAN-SKIP-CHALLENGE"}).then((res) => {
        ...
        if (res.type == 'SUCCESS') {
            ...
            setTimeout(() => {
                chrome.runtime.sendMessage({type: "SEND-CV-TASK-ERROR", data: 'challenge timeout'});
            }, 120000);
        }
    });
...
async function fillingErrors(e, options = {}) {
    await wait(1000);
    if (agentStatus.success) {
        return;
    }
    chrome.runtime.sendMessage({type: 'SET-HAS-MANUAL-FIXES'});
    ...
    if (agentStatus.agentMode == 'Copilot') {
        ...
        if (agentStatus.isApplyOne) {
            ...
            if (agentStatus.disableRemoveButtons || options.returnOnResume) {
                agentStatus.setButtons(2);
            } else {
...
async function pause() {
    while (agentStatus.paused) {
        // Use setTimeout directly (not wait()) to avoid wait→pause→wait infinite recursion
...
const STREAM_LOADER_MAX_ATTEMPTS = 2;
const STREAM_WAIT_INITIAL_MS = 5000;
const STREAM_WAIT_POLL_MS = 500;
const STREAM_WAIT_MAX_POLLS = 230;
const STREAM_RETRY_DELAY_MS = 2000;

function startCountDownInStatusBlock(duration, countDownEnded, options = {}) {
    const maxTotal = Number.isFinite(options.maxTotal) ? options.maxTotal : Infinity;
    const maxStepAdd = Number.isFinite(options.maxStepAdd) ? options.maxStepAdd : Infinity;

    let timer = Math.min(duration, maxTotal);
    let minutes;
    let seconds;
    ...
    function addTime(duration) {
        timer = Math.min(timer + Math.min(duration, maxStepAdd), maxTotal);
    }
...
function logStreamEvent(event, detail) {
    let detailStr = '';
    try {
    ...
    const line = `[STREAM] ${event} ${detailStr}`;
    console.error(line);
    try { debugLog(line); } catch (_) { /* content script may not have debugLog yet */ }
}

async function streamVacancyFields(fields, ustarted) {
    logStreamEvent('streamVacancyFields-call', {
        fieldCount: fields?.length || 0,
        labels: (fields || []).slice(0, 20).map(f => `${f.label || ''}${f.required ? '*' : ''}`),
        url: String(window.location?.href || '').slice(0, 160),
    });
    console.log(fields);
    agentStatus.fieldsRequest = fields
    agentStatus.streamFieldsFailed = false;
    delete agentStatus.streamFailedSimple;
    delete agentStatus.streamFailedComplex;
    ...
    if (simpleFields.length > 0) {
        tasks.push(vacancyFieldsLoader(simpleFields, ustarted, 'simple'));
    }
    if (complexFields.length > 0) {
        tasks.push(vacancyFieldsLoader(complexFields, ustarted, 'complex'));
    }
    if (!tasks.length) return true;
    const results = await Promise.allSettled(tasks);
    const anyOk = results.some(r => r.status === 'fulfilled' && r.value === true);
    if (!anyOk) {
        agentStatus.streamFieldsFailed = true;
        logStreamEvent('stream-all-failed', { fieldCount: fields.length });
    }
    return anyOk;
}
...
async function vacancyFieldsLoader(fields, ustarted, model) {
    ...
    const k_loaderGeneration = 'streamLoaderGeneration' + (model === 'simple' ? 'Simple' : 'Complex');
    const k_fiedsDone = 'fiedsDone' + (model === 'simple' ? 'Simple' : 'Complex');
    const k_fiedsRaw = 'fiedsRaw' + (model === 'simple' ? 'Simple' : 'Complex');
    const k_streamFailed = 'streamFailed' + (model === 'simple' ? 'Simple' : 'Complex');
    ...
    for (let loaderAttempt = 1; loaderAttempt <= STREAM_LOADER_MAX_ATTEMPTS; loaderAttempt++) {
        try {
            ...
            const generation = (agentStatus[k_loaderGeneration] = (agentStatus[k_loaderGeneration] || 0) + 1);
            agentStatus[k_webSocketStarted] = generation;
            ...
            if (!data.role) {
                appendStatusMessage('Error. Role is not parsed correctly.');
            }
        ...
        if (loaderAttempt < STREAM_LOADER_MAX_ATTEMPTS) {
            logStreamEvent('retry-no-response', {
                ...
                attempt: loaderAttempt,
                fieldCount: fields.length,
                labels: fields.slice(0, 15).map(f => f.label),
            });
            appendStatusMessage('No response from the server. Retrying to get the fields...');
            await wait(STREAM_RETRY_DELAY_MS);
        }
    }
    agentStatus[k_streamFailed] = true;
    agentStatus.streamFieldsFailed = true;
    logStreamEvent('stream-give-up', { model, fieldCount: fields.length, attempts: STREAM_LOADER_MAX_ATTEMPTS });
    return false;
}
...
function parseVacancyFieldsJson(js) {
    return JSON.parse(stripJsonComments(js));
}
...
function fuzzyFindField(fields, normalizedLabel) {
    ...
    for (const field of fields) {
        ...
        const candidate = normalizeWhitespace(field.label).toLowerCase();
...
async function getFieldValueByFieldName(label) {
    ...
    if (agentStatus.simpleFields.length > 0) {
        const foundField = agentStatus.fieldsRequest?.find(f => normalizeWhitespace(f.label) === normalizedLabel);
        if (foundField) {
    ...
    while (Date.now() < deadline) {
        if (agentStatus.streamFieldsFailed || agentStatus[k_streamFailed]) {
            logStreamEvent('getFieldValue-abort', { label: normalizedLabel.slice(0, 60) });
        ...
        try {
            ...
            for (let fieldNum = 0; fieldNum < res.length; fieldNum ++) {
                ...
                if (field.label && normalizeWhitespace(field.label) === normalizedLabel) {
            ...
            if (agentStatus[k_fiedsDone]) {
                try {
                    const fuzzyField = fuzzyFindField(res, normalizedLabel);
                    if (fuzzyField) {
                        console.warn('Fuzzy label match:', label, '->', fuzzyField.label);
                        let ret = {value: fuzzyField.value, completed: true};
                        const foundField = agentStatus.fieldsRequest?.find(f => normalizeWhitespace(f.label) === normalizedLabel);
                        if (foundField) {
                            if (!ret.value && ret.value !== 0 && (foundField.required && foundField.required !== 'false') && (foundField.type == 'text' || foundField.type == 'textarea')) {
                                console.warn('Required field is empty (fuzzy):', label, foundField);
                                ret.value = 'N/A';
                            }
                            if (location.hostname.endsWith('workable.com') && foundField.type == 'text' && ret.value && ret.value.length > 127) {
                                ret.value = ret.value.substr(0, 127);
                    ...
                    console.error('Fuzzy match error:', label, e);
                }
    ...
    logStreamEvent('getFieldValue-timeout', { label: normalizedLabel.slice(0, 60) });
    return { value: null, completed: false };
}
...
async function streamMatchScore(details) {
    while (true) {
        ...
        try { ws.close(); } catch {}
        if (!agentStatus.OpenAIConnectionError) {
            logStreamEvent('skip-no-response', { kind: 'match-score' });
...
async function checkMatchScore(details, session, ustarted) {
    if (details?.role?.trim() && !session.platform.endsWith('_BOOKMARKS') && session.platform !== 'ATS' && session.platform !== 'QUEUE' && !["OFF", "Broad match", "Exact match"].includes(session.searchAccuracy)) { 
        ...
        chrome.runtime.sendMessage({type: "SET-MATCH-SCORE", data: score});
...
async function setHistoryDetails(details, ustarted) {
    try {
        ...
        if (r.type != 'SUCCESS') {
            console.error(r);
            if (agentStatus.isApplyOne) {
                appendStatusMessage('You have already applied to this job or the company is ignored.');
                agentStatus.paused = true;
...
async function streamResumePerJob(details) {
    ...
    while (true) {
        ...
        try { ws.close(); } catch {}
        if (!agentStatus.OpenAIConnectionError) {
            logStreamEvent('skip-no-response', { kind: 'resume-per-job' });
...
function collectShadowRootsForLogging(root, depth = 0, path = 'document') {
    if (depth > 20 || !root?.querySelectorAll) {
        return '';
    }
    ...
    for (const el of root.querySelectorAll('*')) {
        try {
            ...
            const elementPath = `${path}/${el.tagName.toLowerCase()}`;
            output += `\n<!-- shadow-root ${elementPath} -->\n<shadow-root path="${elementPath}">\n`;

            try {
                output += el.shadowRoot.innerHTML || '';
            } catch {}

            output += '\n</shadow-root>\n';
            output += collectShadowRootsForLogging(el.shadowRoot, depth + 1, elementPath);
        } catch {}
    }
...
function sendDOMToServer() {
    try {
        let details = document.documentElement.outerHTML;

        try {
            const shadowDump = collectShadowRootsForLogging(document.documentElement);
            if (shadowDump) {
                details += '\n\n<!-- SHADOW ROOTS DUMP -->\n' + shadowDump;
            }
        } catch (e) {
            console.error('shadow root dump failed', e);
        }
...
async function movingOn(data) {
    ...
    if (data.message.includes('ignored company')) {
        return appendStatusMessage('You have already applied to this job or the company is ignored. Skipping and moving on.');
    }
...
async function prepareURL(url, isSearch, allowRedirect) {
    ...
    if (url.startsWith('https://smartapply.indeed.com/') || url.startsWith("https://www.monster.com/jobs/apply-complete") || url.startsWith("https://www.monster.com/profile/apply") || url.startsWith("https://www.monster.com/apply") || url.startsWith("https://www.monster.ca/jobs/apply-complete") || url.startsWith("https://www.monster.ca/profile/apply") || url.startsWith("https://www.monster.ca/apply")) {
        ...
        if (historyDetails?.type == 'SUCCESS' && historyDetails?.data?.url) {
            ...
            console.error('Error getting history details for Monster/Indeed URL normalization:', historyDetails);
            return null;
        }
    }
    ...
    if (url.startsWith('https://www.linkedin.com/')) {
        if (url.startsWith('https://www.linkedin.com/jobs/view/')) {
            ...
            const candidates = document.querySelectorAll('.jobs-apply-button, [data-view-name="job-apply-button"]');
            if (candidates.length > 0) {
                button = candidates[candidates.length - 1];
            }
...
function canApplyHere() {
    ...
    if (location.hostname.includes('myworkdayjobs.com')) {
        const isDescription = /\/job\//.test(location.pathname) && !/\/apply\//.test(location.pathname);
        const isApplyForm = /\/apply\//.test(location.pathname);
        if (!isDescription && !isApplyForm) {
            return false;
        }
        if (document.querySelector('div[data-automation-id="errorContainer"]')) {
            return false;
        }
        if (document.querySelector('div[data-automation-id="invalid-content"]')) {
            return false;
        }
    }
    return true;
...
async function startApplyOne(value, isSearch) {
    try {
        ...
        if (data?.isBoardApply) {
            if (Date.now() - data.isBoardApply > 60000) {
                data.isBoardApply = false;
            } else {
                agentStatus.applyOneWelcome = true;
            }
            const el = await chrome.runtime.sendMessage({type: "RESET-BOARD-APPLY"});
            if (el.data) {
                data.showStartButton = el.data.showStartButton;
        ...
        if (!data?.applyOneEnabled && !data?.isBoardApply) {
        ...
        if (data?.isBoardApply && !data.showStartButton) {
            agentStatus.setButtons(1);
        } else {
        ...
        if (stats?.type == "SUCCESS") {
            if (Number(stats.data.liftsLeft) <= 0 && data.applyOneEnabled != 'QUICK') {
                liftsOut();
                chrome.runtime.sendMessage({type: "STOP-APPLY-ONE"});
                return await startApplyOne(value, isSearch);
            } 
        ...
        await wait(1000);
        appendStatusMessage(`Picking your Profile ${profile.num}: ${profile.title} for auto-apply.`);
        await wait(2000);
        appendStatusMessage(`Resume AI generator per job is ${stats.data.resumePerJob ? 'ON' : 'OFF'}...`)
        await wait(2000);
