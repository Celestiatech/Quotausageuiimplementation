...
def print_lg(*msgs: str | dict, end: str = "\n", pretty: bool = False, flush: bool = False, from_critical: bool = False) -> None:
    ...
    for message in msgs:
        try:
            ...
            # Never let terminal-print issues break bot flow.
            pass
        try:
            with open(__logs_file_path, 'a+', encoding="utf-8", errors="replace") as file:
                file.write(str(message) + end)
        except Exception as e:
            # Do not recurse into logging from here.
            # Keep a minimal, safe fallback to stderr and continue execution.
            try:
                sys.stderr.write(f"[LOG_WRITE_FAILED] {type(e).__name__}: {e}\n")
                sys.stderr.flush()
            except Exception:
                pass
            if not from_critical:
                try:
                    alert(
                        f"Couldn't write to {__logs_file_path}. Close the file if open in Excel/Editor.",
                        "Logging Warning"
                    )
                except Exception:
                    pass
