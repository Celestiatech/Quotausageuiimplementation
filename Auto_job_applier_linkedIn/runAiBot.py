...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    company, work_location = other_details.strip(), ""
    for sep in [" · ", " • ", " Â· "]:
        if sep in other_details:
            company, work_location = [part.strip() for part in other_details.split(sep, 1)]
            break
    if not work_location:
        work_location = search_location.strip() if search_location else ""
    work_style = ""
    if "(" in work_location and ")" in work_location and work_location.rfind("(") < work_location.rfind(")"):
...
def answer_common_questions(label: str, answer: str) -> str:
    if 'sponsorship' in label or 'visa' in label: answer = require_visa
        ...
        ('receive' in label and ('email' in label or 'text' in label or 'phone' in label))
        or 'matching opportunities' in label
        or 'prerecorded voice' in label
    ):
        answer = job_marketing_contact_consent
    return answer

def is_marketing_consent_question(label: str) -> bool:
    return (
        ('receive' in label and ('email' in label or 'text' in label or 'phone' in label))
        or 'matching opportunities' in label
        or 'prerecorded voice' in label
...
def normalize_city_answer(current_city_value: str, work_location: str) -> str:
    """
    Return a city-like value for strict city/location inputs.
    LinkedIn sometimes rejects full "City, State, Country" text in city-only fields.
    """
    source = (current_city_value or "").strip() or (work_location or "").strip()
    if not source:
        return ""
    # Remove parenthetical suffixes like "(Remote)" and keep the first city token.
    source = source.split("(", 1)[0].strip()
    source = source.split(",", 1)[0].strip()
    if not source or source.lower() in {"remote", "united states", "usa", "india", "worldwide"}:
        return ""
    return source
...
def answer_questions(modal: WebElement, questions_list: set, work_location: str, job_description: str | None = None ) -> set:
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            optionsText = [option.text.strip() for option in select.options if option.text]
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                if 'email' in label and not is_marketing_consent_question(label):
                    answer = next((o for o in optionsText if "@" in o), prev_answer)
                ...
                # Add location handling
                    ...
                    if 'country' in label:
                        ...
                        answer = normalize_city_answer(current_city, work_location)
                    else:
                    ...
                    for phrase in possible_answer_phrases:
                        for option in optionsText:
                            ...
                            if phrase.lower() in option.lower() or option.lower() in phrase.lower():
                                ...
                                break
                    if not foundOption:
                        non_placeholder_options = [opt for opt in optionsText if opt.lower() != "select an option"]
                        if len(non_placeholder_options) == 1:
                            select.select_by_visible_text(non_placeholder_options[0])
                            answer = non_placeholder_options[0]
                            foundOption = True
                    if not foundOption:
        ...
        if radio:
            ...
            if overwrite_previous_answers or prev_answer is None:
                ...
                if foundOption: 
                    ...
                    if answer == 'Decline':
                        possible_answer_phrases = ["Decline", "not wish", "don't wish", "Prefer not", "not want", "No", "Opt out", "Do not contact"]
                    elif answer and 'yes' in answer.lower():
                        possible_answer_phrases = ["Yes", "Agree", "I do", "Accept", "Consent", "Opt in"]
                    elif answer and 'no' in answer.lower():
                        possible_answer_phrases = ["No", "Disagree", "I do not", "Decline", "Opt out", "Do not contact"]
                    else:
                        possible_answer_phrases = [answer]
                    ele = options[0]
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                if answer in ["Yes", "No"] and is_marketing_consent_question(label):
                    do_actions = True
...
def close_easy_apply_modal_if_open() -> None:
    '''
    Best-effort modal cleanup to avoid click interception on pagination.
    '''
    try:
        open_modal = try_xp(driver, ".//div[@data-test-modal-id='easy-apply-modal' and @aria-hidden='false']", False)
        if not open_modal:
            return
        actions.send_keys(Keys.ESCAPE).perform()
        wait_span_click(driver, 'Discard', 1)
        wait_span_click(driver, 'Done', 1)
    except Exception as e:
        print_lg("Failed to auto-close leftover Easy Apply modal.", e)
        ...
        try:
            while current_count < switch_number:
                ...
                if pagination_element == None:
                    ...
                    break
                try:
                    close_easy_apply_modal_if_open()
